...
const addToQueue = (installationId, owner, repo, content, token, path) => {
  ...
  const delay = recentTriggers[repository]
    ? 1000 * 60 * (1 + recentTriggers[repository].count)
    : 10
  // delay upcoming deploys
  if (recentTriggers[repository]) {
    clearTimeout(recentTriggers[repository].timer)
    if (recentTriggers[repository].count < 10) {
      recentTriggers[repository].count += 1
    }
  } else {
    recentTriggers[repository] = { count: 1 }
  }
  recentTriggers[repository].timer = setTimeout(() => {
    delete recentTriggers[repository]
  }, 1000 * 60 * (4 + 2 * recentTriggers[repository].count))
  ...
  logger.log(`[${new Date().toISOString()}] delay ${(delay / 1000)}s for ${repository}`)
  ...
  setTimeout(() => {
    ...
    queueItem.timer = setTimeout(async () => {
      ...
      logger.log(`[${new Date().toISOString()}] https://github.com/${repository}`)
    }, 1000 * (isQueued ? 90 : 60))
  }, delay)
}
